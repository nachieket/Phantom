...
from modules.PaloAltoNetworks.TF.TF import TF
...
from modules.MultiThreading.MultiThreading import MultiThreading

from modules.Logger.PoliciesLogger.PoliciesLogger import PoliciesLogger as PLogger
# from ....Logger.ObjectsLogger.ObjectsLogger import ObjectsLogger as OLogger

from modules.Logger.NetworkLogger.NetworkLogger import NetworkLogger as NLogger
from modules.Logger.DeviceLogger.DeviceLogger import DeviceLogger as DLogger

from modules.Logger.SettingsLogger.SettingsLogger import SettingsLogger as SLogger
from modules.Logger.PanoramaGenericLogger.PanoramaGenericLogger import PanoramaGenericLogger as PanLogger


class AWSPrismaAccess(object):
	"""
	...
	def install_panorama(self, parameters):
		...
		try:
			tf.configure_aws_panorama(panorama=parameters['Panorama'], aws=parameters['AWS'], location='stage')
		except Exception as e:
	...
	@staticmethod
	def get_panorama_api_key(public_ip, password):
		thread = MultiThreading()
		ops = Operations()
		...
		while count < 10:
			panorama_api_key = thread.multithread(
				ops.get_api_key, 1, 1, public_ip, password
			)
	...
	def deploy_panorama(self, parameters):
		...
		public_ip = self.install_panorama(parameters=parameters)
		...
		status = self.get_panorama_status(public_ip=public_ip)
		...
		if status == 'up':
			...
			api_key = self.get_panorama_api_key(public_ip=public_ip, password=password)
		...
		while license_count < 5:
			license_status = thread.multithread(
				self.license_panorama, 1, 1, public_ip, api_key, parameters['Panorama']['serial']
			)
		...
		while software_install_count < 5:
			...
			install_job_id = thread.multithread(
				ops.request_panorama_software_install, 1, 1, public_ip, api_key,
				parameters['Panorama']['Panorama_Version'])

			print('Checking installation status\n')

			result, details = thread.multithread(
				ops.request_panorama_job_status, 1, 1, public_ip, api_key, install_job_id)
		...
		while restart_count < 5:
			...
			result = thread.multithread(ops.request_panorama_software_restart, 1, 1, public_ip, api_key)

			if result == 'success':
				status = ops.get_panorama_status(panorama_ip=public_ip)
		...
		if status == 'up':
			...
			system_version = thread.multithread(ops.show_system_info, 1, 1, public_ip, api_key)
		else:
		...
		if system_version == '9.0.6':
			while plugin_download_count < 5:
				...
				status = thread.multithread(ops.request_plugins_check, 1, 1, public_ip, api_key)

				if status == 'success':
					print('Downloading cloud services plugin on Panorama.\n')

					plugin_download_job_id = thread.multithread(
						ops.request_plugins_download_file, 1, 1, public_ip, api_key, 'cloud_services-1.5.0')

					print('Checking cloud services plugin download status.\n')

					result, details = thread.multithread(
						ops.request_panorama_job_status, 1, 1, public_ip, api_key, plugin_download_job_id
					)
		...
		while plugin_install_count < 5:
			print('Installing cloud serviced plugin.\n')

			plugin_install_job_id = thread.multithread(
				ops.request_plugins_install, 1, 1, public_ip, api_key, 'cloud_services-1.5.0'
			)
			...
			result, details = thread.multithread(
				ops.request_panorama_job_status, 1, 1, public_ip, api_key, plugin_install_job_id
			)
			...
			if result == 'OK':
				...
				return 'complete', public_ip, api_key
			else:
	...
	def configure_setup_services(panorama_ip, api_key):
		...
		while answer.lower() != 'done':
			answer = input('Complete cloud service plugin verification and enter \'Done\': ')
	...
	def orchestrator(self, parameters):
		...
		# panorama_ip = '35.179.73.252'
		#
		# api_key = (
		# 	'LUFRPT1PYS9LSjYvY3Z4dmhhTDExWnBrYmZGTms4a3c9VDAzMmIzZmhKalN0K1ZkRXF3SEJsdDdqZ1FaYk1HYVZBbmszamF1eXh'
		# 	'5Z3JwUDRqTTNBTWViR0VBMmxaU2ZzcA=='
		# )
